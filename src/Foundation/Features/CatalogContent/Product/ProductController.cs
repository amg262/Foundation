...
namespace Foundation.Features.CatalogContent.Product
{
    public class ProductController : CatalogContentControllerBase<GenericProduct>
    {
        ...
        public ProductController(IsInEditModeAccessor isInEditModeAccessor,
            ...
            ILoyaltyService loyaltyService) : base(referenceConverter, contentLoader,
            urlResolver, /*reviewService, reviewActivityService,*/ recommendationService, loyaltyService)
        {
        ...
        [HttpGet]
        public async Task<ActionResult> Index(GenericProduct currentContent, string variationCode = "",
            bool skipTracking = false)
        {
            var viewModel =
                _viewModelFactory.Create<GenericProduct, GenericVariant, GenericProductViewModel>(currentContent,
                    variationCode);
        ...
        public ActionResult SelectVariant(string productCode, string color, string size, bool isQuickView = true)
        {
            ...
            if (currentContent != null)
            {
                ...
                {
                    var viewModel =
                        _viewModelFactory.Create<GenericProduct, GenericVariant, GenericProductViewModel>(
                            currentContent, variant.Code);
